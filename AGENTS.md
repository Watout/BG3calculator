...
- `pnpm cicd:apply-github-guardrails` 需要带仓库 `Administration` 权限的 GitHub token，默认直接复用仓库专属变量 `GITHUB_TOKEN_BG3CALCULATOR`；旧的 `GH_ADMIN_TOKEN` / `GITHUB_ADMIN_TOKEN` 只保留兼容。
