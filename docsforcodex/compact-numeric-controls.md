...
这份文档记录桌面端紧凑下拉控件的真实实现位置，以及“执行次数无上限 + 滑动窗口加载”“共享菜单隐藏右侧滚动条”“枚举型选项改走统一按钮式下拉”和“重击阈值最低到 1 但天然 1 仍自动失手、并按降序显示”的代码事实，方便后续继续扩展 UI 或修正数学边界时快速定位。
...
- `CompactSelectDropdown` 统一服务于攻击掷骰状态、目标伤害修正和伤害骰模式。
...
- `App.tsx`
  ...
  - 攻击掷骰状态、目标伤害修正和伤害骰模式也已从原生 `<select>` 切到紧凑下拉；它们复用同一套 trigger / menu / option 样式和关闭逻辑，但不走数值滑动窗口。
- `App.css`
...
- 如果以后继续调整重击阈值顺序、枚举型下拉样式或键盘导航，优先改 `CompactNumericDropdown`、`CompactSelectDropdown` 和 `compactNumericWindow.ts`，不要再退回到静态 `<option>` 全量渲染。
