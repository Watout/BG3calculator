...
这份文档记录桌面端数值型紧凑下拉的真实实现位置，以及“执行次数无上限 + 滑动窗口加载”和“重击阈值最低到 1 但天然 1 仍自动失手”的代码事实，方便后续继续扩展 UI 或修正数学边界时快速定位。
...
- 如果要给重击阈值加入倒序显示或键盘导航，优先改 `CompactNumericDropdown` 和 `compactNumericWindow.ts`，不要再退回到静态 `<option>` 全量渲染。
