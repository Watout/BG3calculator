...
- 数值型紧凑下拉统一走 `apps/desktop-tauri/src/compactNumericWindow.ts`；模板执行次数、主手执行次数、副手执行次数都支持大于等于 `1` 的有限整数，并通过滑动窗口按需加载数值，避免一次性渲染超长选项列表。共享菜单会隐藏右侧滚动条，但保留滚轮与滚动驱动的窗口滑动交互。
- 枚举型下拉中的攻击掷骰状态、目标伤害修正和伤害骰模式，也已切换到与重击阈值相同的紧凑按钮式下拉外观与弹层交互；它们共用同一套 trigger/menu/option 样式和外部点击关闭逻辑，但不使用数值滑动窗口。
- 重击阈值输入同样走紧凑下拉，范围固定为 `1..20`，并按 `20+ -> 1+` 的降序展示；其中 `1+` 仍保留 BG3 / d20 里的天然 `1` 自动失手语义，所以正常单掷时对应的是 `95% crit + 5% miss`，不是 `100% crit`。
...
- “主/副手独立攻击加值表达式”、“副手可空并回退”、“执行次数滑窗下拉”、“攻击掷骰状态/目标伤害修正/伤害骰模式紧凑下拉”、“共享菜单隐藏滚动条”和“重击阈值按降序展示且 `1+` 仍保留天然 1 自动失手”这些输入语义位于 `apps/desktop-tauri` worker 与界面层。
